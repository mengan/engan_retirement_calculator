...
function project(opts) {
  ...
  for (let year = startYear; year <= endYear; year++) {
    ...
    // --- Expenses ---
    const bothWorking = year < s.s1.retireYear && year < s.s2.retireYear;
    const phaseMult = bothWorking
    ...
    const bothRetired = year >= Math.max(s.s1.retireYear, s.s2.retireYear);
    accounts.forEach(a => {
  ...
  expenseByYearChart = new Chart(
    document.getElementById("expenseByYearChart").getContext("2d"),
    {
      ...
      options: {
        responsive: true, maintainAspectRatio: false,
        plugins: {
          title: { display: true, text: "Forecasted Expenses by Year (stacked)" },
          tooltip: { callbacks: { label: c => `${c.dataset.label}: ${fmt(c.parsed.y)}` } },
        },
        scales: {
          x: { stacked: true },
